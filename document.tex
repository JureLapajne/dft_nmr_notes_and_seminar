...
alst step some meaningful density should be constructed. The next step is to
calculate functionals $E_H[n(\vec r)]$ and $E_{xc}[n(\vec r)]$. In the next step
equations are solved and new density is calculated. The latter is then used to
construct new $E_H[n(\vec r)]$ and $E_{xc}[n(\vec r)]$ which are again used to
solve KS equation. The procedure is repeated until the change in density is not
small enough. 
...
determined. $E_{xc}$ is not. It is thus the determining factor for how well KS
equations describe our system. Below, we will have a look at what kind of
functionals are used today.
