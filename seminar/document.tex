...
absorption/adsorption process \cite{introd_to_metal_organ_frameworks}. They can
...
the motion of electrons, sometimes their positions are even fixed. Thus, from now on, we will
...
particle density and effectively reduce the problem to a single particle problem.
...
properties of the system. For non-degenerate cases the latter is uniquely
...
\caption{Translation of the coordinate system for vector $\vec{Ro'}$. $\vec{R_N}$
  points to $N$-th atom, whereas smallc case $\vec r$ point to electrons.}
\end{wrapfigure}

  There are two contributions to shielding tensor - paramagnetic and
  diamagnetic. The former is only present in materials with unpaired electrons
  and should not be confused with hyperfine tensor, also present such materials.
  ...
  \end{equation}
  and a similar expression holds also for paramagnetic contribution $\sigma^p_{\alpha\beta}$. The
  ...
  not depend on origin, but only when one uses complete set of basis function,
