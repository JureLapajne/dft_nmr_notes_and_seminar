...
Local density approximation (lda) are functionals which depend only on
 electron density \cite{challenges_den_fun_theor}.
...
depend not only on electron density but also on its gradient \cite{challenges_den_fun_theor}. Most commonly,
...
atom-centered potentials (DCACPs). The most promising seems to be DFT-D method
...
a consequence DF and DCACP usually cause underbinding \cite{consis_accur_ab_initio_param}.
